...
static uint8_t spi2_read_write_byte(uint8_t txdata);								/* spi读写数据 */
...
{
    __HAL_SPI_DISABLE(&hspi5);                 /* 先关闭SPI2 */
    hspi5.Init.CLKPolarity = SPI_POLARITY_LOW; /* 串行同步时钟的空闲状态为低电平 */
    hspi5.Init.CLKPhase = SPI_PHASE_1EDGE;     /* 串行同步时钟的第1个跳变沿（上升或下降）数据被采样 */
    HAL_SPI_Init(&hspi5);
