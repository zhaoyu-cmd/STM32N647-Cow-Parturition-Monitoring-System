...
extern SRAM_HandleTypeDef hsram1;   /* SRAM句柄 */
...
void lcd_init(void)
{
    FMC_NORSRAM_TimingTypeDef fmc_write_timing_struct = {
        .AddressSetupTime = 15,
        .AddressHoldTime = 15,
        .DataSetupTime = 20,
        ...
        if (lcddev.id != 0x7789)                                                            /* 尝试读取NT35310的ID */
        {
            ...
            if (lcddev.id != 0x5310)                                                        /* 尝试读取ST7796的ID */
            {
                ...
                if (lcddev.id != 0x7796)                                                    /* 尝试读取NT35510的ID */
                {
                    ...
                    lcd_wr_regno(0xC501);
                    lcddev.id |= lcd_rd_data();                                             /* 读取到0x00 */
                    HAL_Delay(5);                                                           /* 因为此NT5510的指令恰好为SSD1963的软件复位指令 */
    ...
    if ((lcddev.id == 0x9341) || (lcddev.id == 0x1963) || (lcddev.id == 0x7789))            /* 9341/1963/7789 */
    {
        ...
        FMC_NORSRAM_Extended_Timing_Init(hsram1.Extended, &fmc_write_timing_struct, hsram1.Init.NSBank, hsram1.Init.ExtendedMode);
    }
